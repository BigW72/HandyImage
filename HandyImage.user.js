...
if (type === 'image' || type === 'video') {
    console.warn(
        "Handy isn't needed for directly opened images or videos (if you want it this way - use CenterImage userscript"
    )
    return false
}
...
if (sessionStorage.length) {
    if (sessionStorage.hji) {
        if (sessionStorage.hji.indexOf(window.location.href) != -1) {
            // check address in case page failed to load b4
            console.warn(
                'Handy Image: userscript stopped itself from running INTENTIONALLY'
            )
        ...
        if (Date.now() < parseInt(sessionStorage.hji_back)) {
            // 2 sec not yet passed
            console.warn(
                'Handy Image: userscript now helps you go to previous page automatically'
            )
...
function protected_addEventListener(el, event, handler, capture = false) {
    return protected_addEventListener.originalF.call(
        el,
        event,
        handler,
        capture
...
if (FireFox && typeof exportFunction === 'function') {
    wrapper_addEventListener = exportFunction(
...
function wrapper_addEventListener(event, handler, capture = false) {
    ...
    _eventHandlers[event].push({
        node: this || unsafeWindow,
        handler: handler,
        capture: capture,
    })
    console.debug(
        'HJI: wrapped 1 addEventListener: ' +
            event +
            ', ' +
            Object.keys(_eventHandlers).length
    )
...
function removeAllListeners() {
    console.debug(
        'HJI: removed ' +
            Object.keys(_eventHandlers).length +
            ' event listeners: ' +
            Object.keys(_eventHandlers)
    )
    for (let event in _eventHandlers) {
        _eventHandlers[event].forEach(({ node, handler, capture }) =>
...
function makeimage() {
    if (typeof cfg_js !== 'string') {
        console.log('waiting for settings to load to makeimage()')
        window.setTimeout(function () {
            makeimage()
        }, 2)
        return false
    } // lets wait for stupd async
    if (cfg_direct === true) {
        let a = protected_createElement('a')
        a.setAttribute('href', i.src)
        a.click()
        return false
    }
    let css = `:root, body
...
:root, body, img, video
{
    ...
    if (dp) {
        console.warn('you are on a double-page image hosting (probably)')
        window.addEventListener('beforeunload', onbeforeunload, true)
    }
    if (typeof GM_registerMenuCommand !== 'undefined') {
        GM_registerMenuCommand(
            'Handy Image Download image',
            download_image,
            'N'
        )
        GM_registerMenuCommand(
...
function find_text_in_scripts(
    text,
    stopword,
    start_from_top = null,
    search_after_word = null,
    content_type = 'img'
) {
    ...
    for (let c = 0; c < s.length; c++) {
        if (
            ...
            s[c].innerHTML = s[c].innerHTML.substring(
                0,
                s[c].innerHTML.indexOf(search_after_word)
            )
        }
        let start_pos = start_from_top
            ? s[c].innerHTML.indexOf(text)
            : s[c].innerHTML.lastIndexOf(text)
        if (start_pos == -1) {
            continue
        }
        start_pos += text.length
        let found_content = s[c].innerHTML.substring(
            start_pos,
            s[c].innerHTML.indexOf(stopword, start_pos)
        )
        found_content = JSON.parse(
            '"' + found_content.replace('"', '\\"') + '"'
        ) // unescape it
...
function makeworld() {
    ...
    if (is_gallery) {
        console.log(
            'Handy Image: userscript stopped itself INTENTIONALLY, - cuz it is not just a single image on the page to fullsize but a gallery'
        )
        observer.disconnect()
        return
    }
    if (i && i.src) {
        ...
        tb = window.setTimeout(function () {
            console.warn(
                'Didnt find image, waited ' +
                    timeout +
                    ' ms to try again. page: ' +
...
function use_booru_tags_in_dl_filename() {
    let artist = document.querySelectorAll(
        ".tag-type-artist a:not([href*='/books?'])[href*='tags='], .artist-tag > a, a.search-tag[itemprop='author'], a.model, .user-info-box .username > a"
    )
    ...
    let character = document.querySelectorAll(
        ".tag-type-character a:not([href*='/books?'])[href*='tags='], .character-tag > a, .character-tag-list a.search-tag, a.character"
    )
    ...
    function do_grab_fav_tags() {
        if (typeof cfg_js !== 'string') {
            console.log(
                'waiting for async setting loading of cfg_js: ' + typeof cfg_js
            )
            tg = window.setTimeout(do_grab_fav_tags, 2)
            return
        }
        if (cfg_js && cfg_js.indexOf('grab_fav_tags') != -1) {
            grab_fav_tags = cfg_js
                .substring(cfg_js.indexOf('[') + 1, cfg_js.indexOf(']'))
                .replaceAll(' ', '')
                .replaceAll('_', ' ')
                .replaceAll(/\n/g, '')
                .replaceAll("'", '')
                ...
                .toLowerCase()
                .split(',')
        } // load custom tags // also bypass CSP
        ...
        if (grab_fav_tags.length) {
            for (let n = 0; n < general_tags.length; n++) {
                ...
                if (
                    grab_fav_tags.indexOf(
                        general_tags[n].text.toLowerCase().replaceAll('_', ' ')
                    ) != -1
                ) {
                    filename =
...
function changeCursor() {
    ...
    if (rescaled === 0) {
        ...
        // fill
        if (orgImgWidth == viewWidth && orgImgHeight == viewHeight) {
            // perfect fit, can't resize
            i.style.cursor = ''
...
function rescale(oEvent, isFilling) {
    ...
    if (oEvent) {
        if (bZoomCenterOnCursor) {
            ...
            window.scrollTo(
                click_X + prevScroll_X - oEvent.clientX,
                click_Y + prevScroll_Y - oEvent.clientY
            )
        } else {
            ...
            window.scrollTo(
                click_X + prevScroll_X - viewWidth / 2,
                click_Y + prevScroll_Y - viewHeight / 2
            )
        }
    } // keep percentage scroll progress for KB hotkeys
    else {
...
function autoresize() {
    if ((!is_video && i.naturalHeight) || i.videoHeight) {
        orgImgWidth = Math.round(
            (is_video ? i.videoWidth : i.naturalWidth) / window.devicePixelRatio
        )
        orgImgHeight = Math.round(
            (is_video ? i.videoHeight : i.naturalHeight) /
        ...
        if (is_video) {
            document.title =
                title + ' (' + i.videoWidth + 'x' + i.videoHeight + ')'
        } else {
            document.title =
                title + ' (' + i.naturalWidth + 'x' + i.naturalHeight + ')'
        }
    ...
    } // no onloadstart event for images, sadge
    else {
        ARC++
        if (ARC < 1000) {
            if (ARC == 75 || ARC == 300 || ARC == 999) {
                // about 1sec, 5sec, 15sec
                i.src = i.src // lol fix
            ...
            console.warn(
                'HJI: Gave up trying to reload the image, it is broken'
            )
...
function scroll_space(a, b) {
    let by = Math.round((b ? viewHeight : viewWidth) * 0.5 * (a ? -1 : 1))
    if (!b) {
        ...
        let details = {
            ...
            headers:
                referrer_policy !== 'no-referrer'
                    ...
                    : {}, // doesn't fully follow the page's referer policy but ok
...
function cfg() {
    if (typeof GM.setValue !== 'undefined') {
        function saveCfg() {
            ...
            if (q('#hji_cfg_2_bgclr').value) {
                document.body.style.backgroundColor =
                    q('#hji_cfg_2_bgclr').value
            } else {
                document.body.style.backgroundColor = ''
            }
        }
        ...
        alert(
            "Sorry, userscripts in browser native mode (without a script manager) can't have configurations! Install TamperMonkey extension. (it's a very good userscript manager)"
        )
    }
}
